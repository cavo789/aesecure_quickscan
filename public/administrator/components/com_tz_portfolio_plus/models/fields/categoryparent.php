		...
		// Initialise variables.
				...
				if ($oldCat == 0)
				{
					foreach ($options as $i => $option)
					{
						...
						if ($user->authorise('core.create', 'com_tz_portfolio_plus.category.' . $option->value) != true )
						{
							unset($options[$i]);
						}
					}
				...
				else
				{
					...
					foreach ($options as $i => $option)
					{
						...
						if ($user->authorise('core.edit.state', 'com_tz_portfolio_plus.category.' . $oldCat) != true)
						{
						...
						elseif
							(($user->authorise('core.create', 'com_tz_portfolio_plus.category.' . $option->value) != true)
							&& $option->value != $oldCat)
						{echo 'x';
							unset($options[$i]);
						}
