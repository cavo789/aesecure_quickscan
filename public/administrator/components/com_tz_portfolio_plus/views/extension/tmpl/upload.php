...
(function($,window){
    ...
    $(document).ready(function(){
        var addonSteps  = [
                {
                    /* Step 1: Upload */
                    element: $(".tpp-extension__upload .btn-extension__upload")[0],
                    intro: "<div class=\\"head\\">'.$this -> escape(JText::_('JTOOLBAR_UPLOAD')).'</div>'
                        .$this -> escape(JText::sprintf('COM_TZ_PORTFOLIO_PLUS_INTRO_GUIDE_UPLOAD_MANUAL_DESC', JText::_('COM_TZ_PORTFOLIO_PLUS_EXTENSION'))).'",
                    position: "right"
                },
                {
                    ...
                    intro: "<div class=\\"head\\">'.$this -> escape(JText::_('COM_TZ_PORTFOLIO_PLUS_INTRO_GUIDE_INSTALL_UPDATE_ONLINE')).'</div>'
                        .$this -> escape(JText::sprintf('COM_TZ_PORTFOLIO_PLUS_INTRO_GUIDE_INSTALL_UPDATE_ONLINE_DESC', JText::_('COM_TZ_PORTFOLIO_PLUS_EXTENSION'))).'",
                    position: "top"
                }];
...
<?php echo JHtml::_('tzbootstrap.addrow');?>
    ...
    <form name="adminForm" method="post" id="adminForm" class="tpp-extension__upload"
          ...
          action="index.php?option=com_tz_portfolio_plus&view=extension&layout=upload">
