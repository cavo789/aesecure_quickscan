...
trait ADatabaseFixmysql
{
	 ...
	 */
	protected function fixHostnamePortSocket(&$host, &$port, &$socket, $useSSL = false)
	{
		...
		// If it's an SSL connection I cannot use a socket
		if ($useSSL)
		{
			$socket = null;

			if (empty($host) || $host === '.' || $host === 'localhost')
			{
				$host = '127.0.0.1';
			}
	...
	/**
	 * Combines a host, port and socket into a single hostname
	 *
	 * @param   string|null $host    Hostname
	 * @param   string|null $port    TCP/IP port
	 * @param   string|null $socket  UNIX socket or Windows Named Pipe
	 * @param   bool        $hasSSL  Is the server connection encryption enabled?
	 *
	 * @return  string  The hostname string which combines all of the above information
	 */
	protected function createMySQLHostname($host, $port, $socket, $hasSSL = false)
	{
		$this->fixHostnamePortSocket($host, $port, $socket, $hasSSL);

		// If SSL is being used we cannot possibly have a socket, we need a port and a host
		if ($hasSSL)
		{
			$socket = null;
			$port = $port ?: 3306;
		...
		// If I have both a port and a socket the socket wins
		if (!empty($port) && !empty($socket))
		{
			$port = null;
		}
		...
		if (empty($host))
		{
			// If I have no host but do have a socket which does not look like Windows I will use a UNIX socket notation
			if (!empty($socket) && substr(trim($socket ?: '', '()'), 0, 4) !== '\\\\.\\')
			{
				// The colon will be added further below
				$host = 'unix';
			}
			// In other cases I fall back to PHP's default behavior: localhost, unless there's a port number
			else
			{
				$host = empty($port) ? 'localhost' : '127.0.0.1';
			}
		}
		elseif ($host === 'localhost' && $hasSSL)
		{
			// If you're connecting over SSL you're using TCP/IP, you have to use 127.0.0.1 instead of localhost
			$host = '127.0.0.1';
		}
		...
		// Special case: Windows sockets
		if (substr(trim($socket ?: '', '()'), 0, 4) === '\\\\.\\')
		{
			$host   = '';
			$prefix = '';
		}
