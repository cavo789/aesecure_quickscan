...
{
	/**
	 * @var    array  A container for AFilterInput instances.
	 */
	 ...
	 * @param   integer  $xssAuto     Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
	 *
	 * @return  AFilterInput  The AFilterInput object.
	 */
	public static function &getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
	{
		...
		{
			self::$instances[$sig] = new AFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
		}
	...
	protected function _cleanAttributes($attrSet)
	{
		...
		for ($i = 0; $i < $count; $i++)
		{
			...
			$explodedAttributeSet = explode(' ', trim($attrSubSet[0]));
			$attrSubSet[0]        = array_pop($explodedAttributeSet);
	...
	protected function _decode($source)
	{
		...
		if (!is_array($ttr))
		{
			...
			{
				$ttr[$v] = AUtilsUtf8::utf8_encode($k);
			}
		}
		$source = strtr($source, $ttr);

		// Convert decimal
		$source = preg_replace_callback('/&#(\d+);/m', function($matches){ return AUtilsUtf8::utf8_encode(chr($matches[0]));}, $source); // decimal notation

		// Convert hex
		$source = preg_replace_callback('/&#x([a-f0-9]+);/mi', function($matches){ return AUtilsUtf8::utf8_encode(chr('0x'.$matches[0]));}, $source); // hex notation
