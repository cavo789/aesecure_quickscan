 ...
 * @since  1.6
 */
abstract class ModArticleSlideshowHelper
{
	/**
	...
	public static function getTagsList($items)
	{
		...
		$tags = array();
		$tagsArray = array();
		$articleTags = new JHelperTags;
		foreach ($items as $key => $item) {
			$itemTags = array();
			$articleTags->getItemTags('com_content.article', $item->id);			
			if(is_array($articleTags->itemTags)) {
				foreach ($articleTags->itemTags as $tag){
					$itemTags[] = trim(strtolower($tag->title));
				}
				$tagsArray[$item->id] = $itemTags;
